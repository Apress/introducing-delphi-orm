...
uses
  ...
  Database.Session.Types, Database.Session, Entities;
...
begin
  ...
  begin
    ...
    try
      session.objectManager.Save(agent);
    except
      if not session.objectManager.IsAttached(agent) then
        agent.Free;
    end;
...
begin
  ...
  begin
    ...
    try
      session.objectManager.Save(department);
    except
      if not session.objectManager.IsAttached(department) then
        department.Free;
    end;
...
begin
  ...
  begin
    ...
    begin
      agent.Description:=Trim(form.edEntity.Text);
      session.objectManager.Flush(agent);
      updateAgents;
    end;
...
begin
  ...
  begin
    ...
    begin
      department.Description:=Trim(form.edEntity.Text);
      session.objectManager.Flush(department);
      updateDepartments;
    end;
...
// Use of Cursors instead of List<>
//
//  for agent in session.objectManager.Find<TAgent>
//                                  .OrderBy('Description')
//                                  .Open do
//  begin
//
...
begin
  ...
  departmentList:=session.ObjectManager.Find<TDepartment>
                                  .OrderBy('Description')
                                  .List;
  // A safer approach is to enclose the following lines in
  // BeginUpdate/EndUpdate and try/finally blocks
