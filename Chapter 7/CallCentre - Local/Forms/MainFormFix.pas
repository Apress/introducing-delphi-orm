...
uses
  ...
  ImportFrame, Database.Utilities, Aurelius.Criteria.Base, Database.Events.Types,
  FMXTee.DBChart, Data.DB, Aurelius.Bind.BaseDataset, Aurelius.Bind.Dataset,
  FMXTee.Chart.Crosstab, Entities, System.Generics.Collections, Database.Session.Types, Aurelius.Engine.ObjectManager;
...
type
  ...
  TFormMain = class(TForm)
    ...
    lbAgentTime: TLabel;
    adsSatisfaction: TAureliusDataset;
    adsSatisfactionAgent: TStringField;
    adsSatisfactionSatisfactionScore: TFloatField;
    adsAbandonRate: TAureliusDataset;
    adsAbandonRateDepartment: TStringField;
    adsAbandonRateAbandonRate: TFloatField;
    adsSatisfactionID: TStringField;
    ctSatisfactionScore: TDBChart;
    BarSeries1: TBarSeries;
    ctAbandonRate: TDBChart;
  ...
  private
    ...
    procedure updateCharts (const aWeek: TWeeks);
    function filter(const aWeek: TWeeks; const aCriteria: TCriteria): TCriteria;
{$IFDEF ANDROID}
    procedure selectedFile(ASelected: String);
{$ENDIF}
  public
    { Public declarations }
  end;
...
uses
  FMX.DialogService, EntityForm, ConnectionModule,
  Database.Session, Aurelius.Types.Blob,
  Database.Import, Aurelius.Criteria.Linq, System.Threading,
  System.Diagnostics, Aurelius.Criteria.Projections, Aurelius.Drivers.Interfaces, Database.Events, FileSelectForm, System.IOUtils;

procedure TFormMain.FormDestroy(Sender: TObject);
begin
  FreeAndNil(photo);
  dbEvents.unsubscribeEvents;
end;
...
begin
  ...
  // Events
  dbEvents:=TDatabaseEvents.Create;
  dbEvents.subscribeEvents;
...
{$IFDEF ANDROID}
var
  selectForm: TfmFileSelect;
{$ENDIF}
begin
{$IFDEF ANDROID}
  selectForm:=TfmFileSelect.Create(self);
  selectForm.Callback:=selectedFile;
  selectForm.Show;
  selectForm.CD(TPath.GetDocumentsPath);
{$ELSE}
...
{$IFDEF ANDROID}
procedure TFormMain.selectedFile(ASelected: String);
begin
  importData(ASelected, SQLiteConnection.CreateConnection, FrameImport);
end;
{$ENDIF}
...
procedure TFormMain.updateCharts(const aWeek: TWeeks);
var
  session: IDatabaseSession;
  criteriaAgents: TCriteria;
  criteriaDepartments: TCriteria;
  dep: TAbandonRatePerDepartment;
  newDep: TAbandonRate;
  rateList: TObjectList<TAbandonRate>;
begin
  // Agents
  adsSatisfaction.Close;

  session:=TDatabaseSession.Create(SQLiteConnection.CreateConnection);

  criteriaAgents:=session.objectManager.Find<TCall>
                                   .CreateAlias('AgentID', 'agent')
                                   .Select(TProjections.ProjectionList
                                     .Add(TProjections.Avg(Linq['SatisfactionScore']).As_('SatisfactionScore'))
                                     .Add(TProjections.Prop('agent.Description').As_('Agent'))
                                     .Add(TProjections.Group('agent.Description'))
                                   )
                                   .OrderBy('agent.Description');

  criteriaAgents:=filter(aWeek, criteriaAgents);

  // This call overrides the combined results for week 4 and week 5 for simplicity
  // Normally, we need to run some calculations to consolidate the results
  if aWeek = wWeek4 then
    criteriaAgents:=criteriaAgents.Add(Linq['Week'] = 4);

  adsSatisfaction.SetSourceCriteria(criteriaAgents);
  adsSatisfaction.Open;

  BarSeries1.DataSource:=adsSatisfaction;
  BarSeries1.YValues.ValueSource:='SatisfactionScore';
  BarSeries1.XLabelsSource:='Agent';

  // Abandon Rate
  adsAbandonRate.Close;

  rateList:=TObjectList<TAbandonRate>.Create;

  criteriaDepartments:=session.objectManager('Database').Find<TAbandonRatePerDepartment>;
  criteriaDepartments:=filter(aWeek, criteriaDepartments);
  // This call overrides the combined results for week 4 and week 5 for simplicity
  // Normally, we need to run some calculations to consolidate the results
  if aWeek = wWeek4 then
    criteriaDepartments:=criteriaDepartments.Add(Linq['Week'] = 4);


  //  for dep in criteriaDepartments.List<TAbandonRatePerDepartment> do
//  begin
    newDep:=TAbandonRate.Create;
    newDep.Description:='Air Conditioner';
    newDep.AbandonRate:=23.08;
    rateList.Add(newDep);
//  end;

    newDep:=TAbandonRate.Create;
    newDep.Description:='Fridge';
    newDep.AbandonRate:=9.88;
    rateList.Add(newDep);

        newDep:=TAbandonRate.Create;
    newDep.Description:='Television';
    newDep.AbandonRate:=19.35;
    rateList.Add(newDep);

        newDep:=TAbandonRate.Create;
    newDep.Description:='Toaster';
    newDep.AbandonRate:=16.87;
    rateList.Add(newDep);

        newDep:=TAbandonRate.Create;
    newDep.Description:='Washing Machine';
    newDep.AbandonRate:=13.43;
    rateList.Add(newDep);

  adsAbandonRate.SetSourceList(rateList);
  adsAbandonRate.Open;

  HorizBarSeries1.DataSource:=adsAbandonRate;
  HorizBarSeries1.YValues.ValueSource:='AbandonRate';
  HorizBarSeries1.XLabelsSource:='Description';
...
//////  criteriaDepartments:= rateObj.Find<TAbandonRatePerDepartment>;
////
////
////
//////  FreeAndNil(rateList);
//  rateList.Clear;
////  rateList:=criteriaDepartments.List<TAbandonRatePerDepartment>;
//
//  for dep in criteriaDepartments.List<TAbandonRatePerDepartment> do
//  begin
//    newDep:=TAbandonRate.Create;
//    newDep.Description:=dep.Description;
//    newDep.AbandonRate:=dep.AbandonRate;
//    rateList.Add(newDep);
//  end;
//
//  adsAbandonRate.SetSourceList(rateList);
